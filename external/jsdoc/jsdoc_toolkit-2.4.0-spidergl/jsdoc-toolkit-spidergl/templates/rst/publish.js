...
function includeTemplate (path, data) {
  ...
  // Provide addtional data
  for (var key in data) {
     JSDOC.JsPlate.keys(key);
     JSDOC.JsPlate.values(data[key]);
     JSDOC.JsPlate.keys['member'] = 'key';
  };

  try {
    template = new JSDOC.JsPlate(publish.conf.templatesDir + path);
  }
  catch(e) { print(e.message); quit(); }
...
function resolveLinks(str, from) {
  str = str.replace(/\{@link ([^} ]+) ?\}/gi,
    function(match, symbolName) {
      return ':doc:`' + symbolName + '<'+symbolName+'>`';
    }
  );
 ...
 * the files
 *
 * @param string text line(s)
 * @depth int amount of padding, default is 4
 * @depth bool indent first line or not, default is false
 */
function reIndent(text, depth, indentFirst) {
  var depth = depth ? depth : 4;
  var indentFirst = indentFirst ? indentFirst: false;
...
/**
 * Function (tries) to convert html into rst 
 */
function restify (html) {
  var rst = "";

  html = html.toString();

  // remove attribute
  rst = html.replace(/\<(\/?\w*)([^\>]*)\>/g, '<$1>');
  ...
  // bold
  rst = rst.replace(/\<.?b\>/g, "**");
  rst = rst.replace(/\<.?strong\>/g, "**");

  // code
  rst = rst.replace(/\<.?code\>/g, "``");
  rst = rst.replace(/\<.?pre\>/g, "``");

  // unicode
  rst = rst.replace('&#64;', '@');

  // blankrow/break
  rst = rst.replace(/\<br(\s|\/)*\>/g, '\n\n');

  // eat divs
  // TODO: consider using containers?
  rst = rst.replace(/\<.?div\>/g, "\n");
